      ...
      <AnimatePresence>
        {!open && (
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={onOpen}
            className="absolute bottom-0 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 px-5 py-2.5 rounded-t-xl border border-b-0 border-hairline bg-surface/95 backdrop-blur-sm shadow-[0_-4px_24px_rgba(0,0,0,0.08)] text-xs font-semibold text-ink hover:bg-surface hover:border-brand/30 hover:text-brand transition-colors cursor-pointer"
            aria-expanded={false}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute inset-x-0 bottom-0 z-40 flex flex-col bg-surface border-t border-hairline shadow-[0_-8px_30px_rgba(0,0,0,0.1)] overflow-hidden"
          ...
          >
            <motion.div
              ...
              className="shrink-0 flex items-center justify-between px-4 py-2 border-b border-hairline bg-canvas"
            >
              <div className="flex items-center gap-2 min-w-0">
                <Braces className="w-4 h-4 text-brand shrink-0" />
                <span className="text-xs font-semibold text-ink">Diagram Code</span>
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <button
                  ...
                  className={`text-[10px] px-2.5 py-1 rounded-md border font-medium inline-flex items-center gap-1 ${
                    showSyntax ? 'bg-brand/10 border-brand/30 text-brand' : 'border-hairline text-ink-muted hover:bg-surface'
                  }`}
                >
                ...
                <button
                  ...
                  className="text-[10px] px-2.5 py-1 rounded-md border border-hairline text-ink-muted hover:bg-surface font-medium"
                >
                  This journey
                </button>
                <button
                ...
                <button
                  ...
                  className="text-[10px] px-2.5 py-1 rounded-md border border-hairline text-ink-muted hover:bg-surface font-medium inline-flex items-center gap-1"
                >
                ...
                <button
                  ...
                  disabled={applying}
                  onClick={handleApply}
                  className="text-[10px] px-3 py-1 rounded-md bg-brand text-canvas font-semibold inline-flex items-center gap-1 disabled:opacity-50"
                >
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="w-7 h-7 rounded-md flex items-center justify-center text-ink-muted hover:bg-surface-hover"
            ...
            >
              <textarea
                ...
                spellCheck={false}
                className="flex-1 resize-none p-4 font-mono text-[12px] leading-relaxed text-ink bg-[#FDFDFB] focus:outline-none focus:ring-2 focus:ring-inset focus:ring-brand/20"
                placeholder={'Paste Mermaid flowchart code here…\n\nflowchart TD\n    A[Start] --> B{Decision?}'}
              />
              {showSyntax && (
                <pre className="w-72 shrink-0 border-l border-hairline bg-canvas p-3 overflow-auto text-[10px] text-ink-muted font-mono leading-relaxed whitespace-pre-wrap">
